...
export default function ProfilePage() {
  ...
  const allDimensionsCompleted =
    hasScaleAnswers && hasMajorFavorites && hasProvinceFavorites && hasChoices;
  ...
  // 计算评估状态
  const assessmentStatus: AssessmentStatus = allDimensionsEmpty
    ? 'not_started'
  ...
  useShareAppMessage(() => {
    ...
    const path = uuid
      ...
      : '/pages/index/index';
    return {
      title: '逆袭智愿 - 喊上那个同频的TA，带着你们的天赋，去探索属于你们喜欢的专业！',
      path,
        ...
        <View className="profile-page__content">
            ...
            <View className="profile-page__card-body">
              ...
              <View
                className={`profile-page__card-item ${
                  scaleAnswersCount < TOTAL_QUESTIONS && repeatCount === 0
                    ? 'profile-page__card-item--disabled'
                    : ''
                }`}
                onClick={handleRestartAssessment}
              >
                <View
                  className={`profile-page__card-icon ${
                    scaleAnswersCount >= TOTAL_QUESTIONS
                      ? 'profile-page__card-icon--restart'
                      : repeatCount > 0
                      ? 'profile-page__card-icon--continue'
                      : 'profile-page__card-icon--disabled'
                  }`}
                >
                  <Text className="profile-page__card-icon-text">
                    {scaleAnswersCount >= TOTAL_QUESTIONS ? '🔄' : repeatCount > 0 ? '📝' : '🔄'}
                  </Text>
                </View>
                <View className="profile-page__card-item-content">
                  <Text
                    className={`profile-page__card-item-title ${
                      scaleAnswersCount < TOTAL_QUESTIONS && repeatCount === 0
                        ? 'profile-page__card-item-title--disabled'
                        : ''
                    }`}
                  >
                    {scaleAnswersCount >= TOTAL_QUESTIONS
                  ...
                  <Text
                    className={`profile-page__card-item-desc ${
                      scaleAnswersCount < TOTAL_QUESTIONS && repeatCount === 0
                        ? 'profile-page__card-item-desc--disabled'
                        : ''
                    }`}
                  >
                    {scaleAnswersCount >= TOTAL_QUESTIONS
                ...
                <Text
                  className={`profile-page__card-arrow ${
                    scaleAnswersCount < TOTAL_QUESTIONS && repeatCount === 0
                      ? 'profile-page__card-arrow--disabled'
                      : ''
                  }`}
                >
                  ›
              ...
              <View
                className={`profile-page__card-item ${
                  scaleAnswersCount < TOTAL_QUESTIONS ? 'profile-page__card-item--disabled' : ''
                }`}
              ...
              >
                <View
                  className={`profile-page__card-icon ${
                    scaleAnswersCount >= TOTAL_QUESTIONS
                      ? 'profile-page__card-icon--report'
                      : 'profile-page__card-icon--disabled'
                  }`}
                >
                ...
                <Text
                  className={`profile-page__card-arrow ${
                    scaleAnswersCount < TOTAL_QUESTIONS ? 'profile-page__card-arrow--disabled' : ''
                  }`}
                >
                  ›
                </Text>
          ...
          <Card className="profile-page__card">
            ...
            <View className="profile-page__card-body">
              ...
              {dataLoaded &&
                (relatedDataUserType === 'admin' ||
                  relatedDataUserType === 'promoter' ||
                  relatedDataUserType === 'test') && (
                  <View
                    className="profile-page__card-item"
      ...
      >
        <DialogContent className="profile-page__promote-dialog" showCloseButton={!promoteSaving}>
          ...
          <View className="profile-page__promote-actions">
            <Button disabled={promoteSaving} onClick={handlePromoteDownload}>
